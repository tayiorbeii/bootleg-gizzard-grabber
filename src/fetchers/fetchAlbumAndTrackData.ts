...
export async function fetch2024Albums(
  artistId: string
  ...
  return allAlbums
    ...
    .sort(
      (a: SpotifyAlbum, b: SpotifyAlbum) =>
        new Date(b.release_date).getTime() - new Date(a.release_date).getTime()
    );
}

export async function fetchDetailedAlbumData(
  albums: SpotifyAlbum[]
): Promise<DetailedAlbumData[]> {
  const accessToken = await getSpotifyAccessToken();
  const detailedAlbums = await getDetailedAlbums(
    accessToken,
    albums.map((a) => a.id)
  );
...
export function createSimplifiedOutput(
  ...
  const simplifiedAlbums: SimplifiedAlbum[] = detailedAlbumData.map((album) => {
    const simplifiedTracks: SimplifiedTrack[] = album.detailed_tracks.map(
      (track) => ({
        name: track.name,
...
export function getTopTracksFromAlbum(
